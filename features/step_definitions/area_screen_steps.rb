Given(/^I land on "([^"]*)" popup$/) do |value|
  find_element(id: "title_template").find_element(xpath: "//android.widget.TextView[@text='#{value}']")
end
...
Then(/^I land on "([^"]*)" screen$/) do |value|
  ...
  if actual_value != value
   fail("Error! No need of the item. There is something wrong with the screen.")
  end
end
